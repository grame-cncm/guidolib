...
void GuidoPianoRoll::DrawOctavesGrid(GuidoPianoRoll::DrawParams drawParams) const
{
	for (int i = fLowPitch; i <= fHighPitch + 1; i++) {
        float y = pitch2ypos(i, drawParams.height) + 0.5f * drawParams.noteHeight;
        ...
        if (step == 0) { // C notes are highlighted
            ...
            drawParams.dev->Line(drawParams.untimedLeftElementWidth, y, (float) drawParams.width, y);
...
void GuidoPianoRoll::DrawKeyboard(GuidoPianoRoll::DrawParams drawParams) const
{
    ...
    float yMin = pitch2ypos(fLowPitch, drawParams.height)  + 0.5f * drawParams.noteHeight;
    float yMax = pitch2ypos(fHighPitch, drawParams.height) - 0.5f * drawParams.noteHeight;
    drawParams.dev->Line(0, yMin, 0, yMax);
    ...
    for (int i = fLowPitch; i <= fHighPitch + 1; i++) {
        ...
        case 0:
            if (i != fHighPitch + 1) {
                if (i == 60) { // Tint C4 note in grey
                    ...
                    float xCoords[6] = {
                        ...
                        keyboardBlackNotesWidth,
                        drawParams.untimedLeftElementWidth,
                        drawParams.untimedLeftElementWidth,
                        0
                    };
            ...
            drawParams.dev->Line(0, y, drawParams.untimedLeftElementWidth, y);
        ...
        case 10:
            if (i != fHighPitch + 1)
                drawParams.dev->Rectangle(0, y - drawParams.noteHeight, keyboardBlackNotesWidth, y);

            break;
        }
    }
    
    drawParams.dev->Line(drawParams.untimedLeftElementWidth, yMin, drawParams.untimedLeftElementWidth, yMax);
...
void GuidoPianoRoll::DrawNote(int pitch, double date, double dur, GuidoPianoRoll::DrawParams drawParams)
{
	...
	float y = pitch2ypos(pitch, drawParams.height);
...
void GuidoPianoRoll::DrawRect(float x, float y, double dur, GuidoPianoRoll::DrawParams drawParams) const
{
    ...
    drawParams.dev->Rectangle(x, y - halfstep, x + (w ? w : 1), y + halfstep);
}
...
void GuidoPianoRoll::DrawMeasureBar(double date, GuidoPianoRoll::DrawParams drawParams) const
{
    ...
    drawParams.dev->Line(x, yMin, x, yMax);
    drawParams.dev->PopPenWidth();
...
//--------------------------------------------------------------------------
float GuidoPianoRoll::pitch2ypos(int midipitch, int height) const
{
	int   p = midipitch - fLowPitch;
    float h = ((float) (height * p) / (float) pitchRange());
