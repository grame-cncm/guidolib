...
\item \guidotag{cresc}/\guidotag{decresc} : paramétrable en position, épaisseur, couleur.
\end{itemize}
...
Enfin, concernant le \emph{feathered beam} comme le \emph{beam} classique, il est maintenant possible de créer des hiérarchies de \emph{beams} : c'est à dire d'englober plusieurs \emph{beams} dans un plus grand, afin de les joindre par une barre principale commune (Figure \ref{fig:fbeamhierarchie}).
