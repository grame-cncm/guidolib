...
Release 0.3 of the GUIDO NoteViewer presents the following
new features:

* new automatic system break-algorithms
  based on a refined spring-rod-modell
* new algorithms for automatic beaming,
  automatic clef, key and meter-settings
* new algorithms that synchronize events 
  and tags horizontally in different voices 
  and staves
* empty event is supported
* supports different units for tagparameters
  (mm, cm, in, pt, pc ...)
* default unit is hs (Halfspace)
* automatic features can be turned off
  (\autoBreak<"off"> ... )
* better (automatic) tuplet-handling 
  (explicit tuplet-tag is now supported)
* the \merge-tag is now supported
* the \bembel tag is supported ( :-) )
* supports tag-id's and more Begin- and End-Tags
* better support for tags that reach beyond 
  systems
...
GUIDO NoteViewer, Version 0.3 needs a PC with 
...
GUIDO NoteViewer, Version 0.3 is still a prototype. Not all of the 
